"""OpenAI-Responses-compatible MAQCache proxy with JEV semantic matching.
...
from maqcache.similarity_evaluation.jev import reuse_questions
...
UPSTREAM = os.getenv("MAQCACHE_UPSTREAM_URL", "https://api.openai.com/v1")
UPSTREAM_KEY = os.getenv("MAQCACHE_UPSTREAM_KEY", "")
MODEL = os.getenv("MAQCACHE_UPSTREAM_MODEL", "deepseek-v4-flash")
JEVI_URL = "https://api.typesafe.ai/v1/systemone"
JEVI_KEY = os.getenv("JEV")
THRESHOLD = float(os.getenv("MAQCACHE_THRESHOLD", "0.70"))
PORT = int(os.getenv("MAQCACHE_PORT", "8765"))
...
def build_cached_response(text, model):
    return {
        ...
        "model": model,
        "status": "completed",
        "output": [{"type": "message", "id": "msg_maqcache", "status": "completed",
                    "role": "assistant",
    ...
    def do_GET(self):
        ...
        if path.endswith("/models"):
            data = json.dumps({"object": "list", "data": [{"id": MODEL, "object": "model",
                                                            "created": 0, "owned_by": "maqcache"}]}).encode()
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", str(len(data)))
            self.end_headers()
...
if __name__ == "__main__":
    print(f"MAQCache proxy (JEV semantic) listening on http://localhost:{PORT}/v1", flush=True)
