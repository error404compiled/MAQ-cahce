...
from maqcache.manager.scalar_data.base import CacheStorage, CacheData, Question, QuestionDep, Answer
...
class DynamoStorage(CacheStorage):
    ...
    def create(self):
        ...
        if not self._does_table_already_exist_and_is_active("maqcache_questions"):
            self._dynamo.create_table(
                TableName = "maqcache_questions",
                KeySchema = [
        ...
        if not self._does_table_already_exist_and_is_active("maqcache_reports"):
            self._dynamo.create_table(
                TableName = "maqcache_reports",
                KeySchema = [
    ...
    def batch_insert(self, all_data: List[CacheData]) -> List[str]:
        ...
        """
        table = self._dynamo.Table("maqcache_questions")
    ...
    def report_cache(
        ...
        table = self._dynamo.Table("maqcache_reports")
        table.wait_until_exists()
