...
class ObjectBase:
    ...
    def get(name, **kwargs):
        if name == "local":
            from maqcache.manager.object_data.local_storage import LocalObjectStorage  # pylint: disable=import-outside-toplevel
            object_base = LocalObjectStorage(kwargs.get("path", "./local_obj"))
        elif name == "s3":
            from maqcache.manager.object_data.s3_storage import S3Storage  # pylint: disable=import-outside-toplevel
