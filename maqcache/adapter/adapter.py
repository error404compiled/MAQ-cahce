...
from maqcache.processor.post import temperature_softmax, LlmVerifier
...
def adapt(llm_handler, cache_data_convert, update_cache_callback, *args, **kwargs):
    ...
    if cache_enable:
        try:
            ...
            maqcache_log.warning("failed to save the data to cache, error: %s", e)
    return llm_data
...
async def aadapt(
    ...
    if cache_enable:
        try:
            ...
            maqcache_log.error("failed to save the data to cache", exc_info=True)
    return llm_data
...
def _summarize_input(text, text_length):
    ...
    from maqcache.processor.context.summarization_context import (
...
def cache_health_check(vectordb, cache_dict):
    ...
    if not flag:
        maqcache_log.critical("Cache Store and Vector Store are out of sync!!!")
