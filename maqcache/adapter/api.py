...
import maqcache.processor.post
import maqcache.processor.pre
from maqcache import Cache, cache, Config
from maqcache.adapter.adapter import adapt
from maqcache.embedding import (
    Onnx,
    Huggingface,
    SBERT,
    FastText,
...
from maqcache.embedding.base import BaseEmbedding
from maqcache.manager import manager_factory
from maqcache.manager.data_manager import DataManager
from maqcache.processor.context import (
...
from maqcache.similarity_evaluation import (
...
def put(prompt: str, data: Any, **kwargs) -> None:
    """put api, put qa pair information to MAQCache
    ...
    Example:
        .. code-block:: python
            ...
            from maqcache.adapter.api import put
            from maqcache.processor.pre import get_prompt
...
def init_similar_cache_from_config(config_dir: str, cache_obj: Optional[Cache] = None):
    ...
    storage_config.setdefault("data_dir", "maqcache_data")
...
def _get_pre_func(pre_process):
    return getattr(maqcache.processor.pre, pre_process)
...
def _get_post_func(post_process):
    return getattr(maqcache.processor.post, post_process)
