...
from maqcache.adapter.api import (
    get,
    put,
    init_similar_cache,
...
from maqcache.utils import import_fastapi, import_pydantic, import_starlette
...
async def chat(request: Request):
    if openai_cache is None:
        raise HTTPException(
            status_code=500,
            detail=f"the maqcache server doesn't open the openai completes proxy",
        )
...
def main():
    ...
    parser.add_argument(
        "-d", "--cache-dir", default="maqcache_data", help="the cache data dir"
    )
